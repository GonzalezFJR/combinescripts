...
#combine  ${POIS} -M MultiDimFit -t -1 --rMin 0.1 --rMax 2.0 ${CARD}
...
combine -M MultiDimFit --algo=grid --points=${SCAN} ${POIS} ${CARD}  -n Nominal --rMin 0.1 --rMax 2.0
...
combine -M MultiDimFit --algo grid --points ${SCAN} --rMin 0.1 --rMax 2.0 -n Systs higgsCombineBestfit.MultiDimFit.mH120.root --snapshotName MultiDimFit --freezeParameters "Lumi,tW,WJets,QCD,DY,VV,Nonprompt,lepSF,elecSF,muonSF,btagSF,FSR,ISR,JES,JER,prefire,PDF,Scales,trigSF,trigElecSF,trigMuonSF,hdamp,UE,ch,ch autoMCStats" ${POIS}
